...
void initializeStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	...
	for (int i = 0; i < NUM_STRINGS*STRING_LENGTH; ++i) {	//intializes the strings and creates a null terminator
		*(ptr + i) = NULL;
	}
}
...
void printStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	...
	for (int i = 0; i < NUM_STRINGS * STRING_LENGTH; i++) {		//loop to create string
...
char* reverseOneString(char s[STRING_LENGTH])
{
	...
	int len = strlen(s),loops;
	// Enter the code below
	if (len % 2) {
		loops = (len / 2) + 1;	// for odd string length
	}
	else {
		loops = len / 2;		//for even string length
	}
	for (int i = 0; i < loops; i++) {  // swaps.
		temp = *(ptr+ i);
		*(ptr + i) = *(ptr + len - 1 - i);
		*(ptr + len - 1 - i) = temp;
	}
...
void reverseStrings(char strings[NUM_STRINGS][STRING_LENGTH])
{
	...
	for (int i =0; i < NUM_STRINGS-1; i++) {	//reverses the string using reverseoneString
		if (len != 0) {
...
void encryptStrings(char strings[NUM_STRINGS][STRING_LENGTH], int key)
{
	...
	// Enter the code below
	int i;
	for (i = 0; i < NUM_STRINGS * STRING_LENGTH; i++) {	//loop
		if(*(ptr + i) != '\0') {	// reverses the char until it hits \0
			reverseStrings(ptr);	// call revers string
			*(ptr + i) = *(ptr + i) + key;	//encryptes the string with the certain number of user input
		}
	}
...
void decryptStrings(char strings[NUM_STRINGS][STRING_LENGTH], int key)
{
	...
	for (int i = 0; i < NUM_STRINGS * STRING_LENGTH; i++) {	//loop
		if(*(ptr + i) != '\0') {	//goes to next string
			reverseStrings(ptr);	//reverts the string 
			*(ptr + i) = *(ptr + i) - key;	//decodes
		}
	}
}
...
int isPalindrome(char s[STRING_LENGTH])
{
	...
	// Enter the code below
	while (ptr < last) {
		if (*ptr == *last) {	// if true then it returns a 1 which means it is a palindrome
			return 1;
			break;		//break
		}
		ptr++;		//increments
	...
	return 0;		//retunrns 0 if not a palindrome
}
