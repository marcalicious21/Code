...
void addBook(string name_input, double price_input, libraryType type) 
{
	...
	Container* newCont = new Container();  //creating a new container
	
	Book* bookType;                              // creating a newbook
	if (type == 1) { //goes to hayden
		bookType = new Noble(name_input, price_input, type);
	}
	else {
		bookType = new Hayden(name_input, price_input, type);	//else goes to Hayden.
	}
	
	newCont->book = bookType;  //putting the book into a new container
	newCont->next = NULL;		// setting the new container to null

	if (list == NULL) {	//checks if the list is empty
		list = newCont;
	}
	else {				//when not empty, adds the new container into the tail
		while ((tempList->next) != NULL) {
			tempList = tempList->next;
		}
		tempList->next = newCont;
	}
...
void save(string fileName)
{
	...
	if (myFileVar.is_open()) {
		tempList = list;
		myFileVar << counter << endl;
		while (tempList != NULL) {
			myFileVar << tempList->book->getName() << endl; //saves the name of the book
			myFileVar << tempList->book->getPrice() << endl; // saves the price
			int typeint = int(tempList->book->getLibraryType()); // converts libtype into a typeint
			myFileVar << typeint << endl;//saves the typeint
...
void load(string fileName)
{
	...
	if (myFileVar.is_open()) { 
		myFileVar >> counter;
		for (i = 0; i < counter; ++i) {       
			string name; double price; int libtype;
			myFileVar >> name; // loads the book name
			myFileVar >> price; // loads the price
			myFileVar >> libtype; // load the book type
			libraryType type = libraryType(libtype);//covenverts int back to type
			addBook(name, price, type);// use add fuction the add the new book
		}
		myFileVar.close();
	}
}
